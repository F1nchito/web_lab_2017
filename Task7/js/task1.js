    ...
    function parseString(input) {
        ...
        for (i = 0;; i++) {
            if (i % 2 === 0) {
                ...
                if (input.substring(0, matches[i].index).match(patterns.operator) !== null) {
                    throw new Error("2nd operator between numbers");
                }
                if (input.substring(0, matches[i].index).match(/\./) !== null) {
                    throw new Error("'.' not allowed between number and operator");
                }
                ...
                if (input.substring(0, matches[i].index).match(patterns.numbers) !== null) {
                    throw new Error("2nd number between operator and number");
                }
                if (matches[i][0] == "=") {
                    break;
                }
